...
const dbroute = process.env.MONGODB_URL || 'mongodb+srv://admin:admin@cluster0.vfeta.mongodb.net/shoes?retryWrites=true&w=majority';
...
server.get('/api/shoes/:id', (req, res) => {
  db.collection('shoes').findOne({_id: new ObjectID(req.params.id) }, (err, result) => {
    if (err) throw err;
...
server.delete('/api/shoes', (req, res) => {
  db.collection('shoes').deleteOne( {_id: new ObjectID(req.body.id) }, err => {
...
server.post('/api/shoes', (req, res) => {
  db.collection('shoes').insertOne(req.body, (err, result) => {
    if (err) throw err;
...
server.put('/api/shoes', (req, res) => {
  // get the ID of the user to be updated
  const id  = req.body._id;
  // remove the ID so as not to overwrite it when updating
  delete req.body._id;
  // find a user matching this ID and update their details
  db.collection('shoes').updateOne( {_id: new ObjectID(id) }, {$set: req.body}, (err, result) => {
    if (err) throw err;
